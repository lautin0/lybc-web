...
export default function PersonalSearchAppBar() {
   ...
   const renderMenu = (
      ...
      >
         ...
         {(tokenPair && hasRole(tokenPair.token, [Role.Super, Role.Admin, Role.Worker])) && <MenuItem onClick={() => history.push('/admin/')}>管理控制台</MenuItem>}
         <MenuItem
            onClick={() => {
               signOut()
      ...
      >
         ...
         <MenuItem onClick={handleProfileMenuOpen} alignItems="center">
            ...
            >
               ...
               {(!loading && profilePicData?.user?.profilePicURI) && <Avatar className={classes.small} alt="profile pic" src={UNIVERSALS.GOOGLE_STORAGE_ENDPOINT + profilePicData?.user.profilePicURI} />}</IconButton>
